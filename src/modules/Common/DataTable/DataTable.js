...
const EnhancedTableToolbar = props => {
    ...
    function selectFilterOption(event, filter) {
    ...
    return (
        ...
        >
            ...
            <Menu
                ...
                onClose={()=>setFilterDropdownAnchor(null)}
            >
                <li>
                    <Typography
                        color="textSecondary"
                ...
                { filters && filters.map((filter, idx) => {
                    return (<MenuItem onClick={(event)=>{selectFilterOption(event,filter)}} key={idx}>{ filter.label }</MenuItem>)
                })}
            </Menu>
            ...
            <Menu
                ...
                onClose={()=>setFilterOptionDropdownAnchor(null)}
            >
                { selectedFilter && selectedFilter.type === "search" && (
                ...
                { selectedFilter && selectedFilter.type === "option" && (<li style={{padding:'3px 15px'}}> { selectedFilter.label } </li>) }
                { selectedFilter && selectedFilter.type === "option" && selectedFilter.options.map((opt,idx) => <MenuItem onClick={()=> {optionFilter(opt.value)}} key={idx}>{ opt.label }</MenuItem>) }
...
export default function EnhancedTable(props) {
    ...
    const key = props.keyProp || props.head[0].id;
